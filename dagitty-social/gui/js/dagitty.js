...
var Graph = Class.extend({ 
	...
	managed_vertex_property_names : ["source","target","adjustedNode",
		"latentNode","selectionNode","question","input","answer","decision","result"],
	init : function(){
			...
			if( s.type == "node" && s.id == "graph" ){
				...
				for( i = 0 ; i < s.attributes.length ; i ++ ){
					...
					case "question":
						...
						break
					case "questionlabel":
						n.questionlabel = s.attributes[i][1]
						break
					case "input":
						g.addInput( n )
						break
					case "inputlabel":
						n.answerlabel = s.attributes[i][1]
						break
					case "inputdefaultvalue":
						n.inputdefaultvalue = s.attributes[i][1]
						break
					case "inputsort":
						n.inputsort = s.attributes[i][1]
						break
					case "answer":
						g.addAnswer( n )
						break
					case "answervalue":
						n.answervalue = s.attributes[i][1]
						break
					case "answerlabel":
						n.answerlabel = s.attributes[i][1]
						break
					case "decision":
						g.addDecision( n )
						break
					case "decisioncalculation":
						n.decisioncalculation = s.attributes[i][1]
						break
					case "result":
						g.addResult( n )
						break
					case "resultlabel":
						n.resultlabel = s.attributes[i][1]
						break
					case "resultcalculation":
						n.resultcalculation = s.attributes[i][1]
						break
					case "addlinfo":
						n.addlinfo = s.attributes[i][1]
						break
					case "answersort":
						n.answersort = 	s.attributes[i][1]
						break
					case "pos":
...
var ObservedGraph = Class.extend({
	...
	event_mapping : {
		...
		"addQuestion" : "change",
		"removeQuestion" : "change",
		"addInput" : "change",
		"removeInput" : "change",
		"addAnswer" : "change",
		"removeAnswer" : "change",
		"addDecision" : "change",
		"removeDecision" : "change",
		"addResult" : "change",
		"removeResult" : "change"
	},
...
var GraphSerializer = {
	...
	toDotVertexStatements : function( g ){
		var expandLabel = function( v, g ){
			...
			g.isQuestion(v) && properties.push("question")
			g.isInput(v) && properties.push("input")
			g.isAnswer(v) && properties.push("answer")
			g.isDecision(v) && properties.push("decision")
			g.isResult(v) && properties.push("result")

			if( g.isQuestion(v) && v.questionlabel ) {
				properties.push ("questionlabel=\"" +
					v.questionlabel + "\"")
			}

			if( g.isInput(v) && v.inputlabel ) {
				properties.push ("inputlabel=\"" +
					v.inputlabel + "\"")
			}

			if( g.isInput(v) && v.inputdefaultvalue ) {
				properties.push ("inputdefaultvalue=\"" +
					v.inputdefaultvalue + "\"")
			}

			if( g.isInput(v) && v.inputsort ) {
				properties.push ("inputsort=\"" +
					v.inputsort + "\"")
			}

			if( g.isAnswer(v) && v.answerlabel ) {
				properties.push ("answerlabel=\"" +
					v.answerlabel + "\"")
			}

			if( g.isAnswer(v) && v.answervalue) {
				properties.push ("answervalue=\"" +
					v.answervalue + "\"")
			}

			if( g.isAnswer(v) && v.answersort) {
				properties.push ("answersort=\"" +
					v.answersort + "\"")
			}

			if( g.isDecision(v) && v.decisioncalculation ) {
				properties.push ("decisioncalculation=\"" +
					v.decisioncalculation + "\"")
			}

			if( g.isResult(v) && v.resultlabel ) {
				properties.push ("resultlabel=\"" +
					v.resultlabel + "\"")
			}

			if( g.isResult(v) && v.resultcalculation ) {
				properties.push ("resultcalculation=\"" +
					v.resultcalculation + "\"")
			}

			if(v.addlinfo) {
				properties.push ("addlinfo=\"" +
					v.addlinfo + "\"")
			}
	...
	drawGraph : function(){
		...
		for( i = 0 ; i < vv.length ; i ++ ){
			...
			if( g.isSource(vv[i]) ){
				...
				vertex_type = "latent"
			} else if( g.isQuestion(vv[i]) ) {
				vertex_type = "question"
			} else if( g.isInput(vv[i]) ) {
				vertex_type = "input"
			} else if( g.isAnswer(vv[i]) ){
				vertex_type = "answer"
			} else if( g.isDecision(vv[i]) ) {
				vertex_type = "decision"
			} else if( g.isResult(vv[i]) ){
				vertex_type = "result"
	...
	*/
	...
	setVertexProperty : function( _v, p ){
		...
		if( v ){
			...
			_.each(["Source","Target","AdjustedNode","LatentNode","Question","Input","Answer","Decision","Result"],function(pc){
